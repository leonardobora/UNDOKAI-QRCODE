...
from models import Participant, EmailLog
...
def send_single_email(participant, dry_run=False):
    ...
    try:
        ...
        if dry_run:
            logger.info(f"[DRY RUN] Would send email to {participant.nome} ({participant.email})")
            return True
...
def send_batch_emails(batch_size=10, delay=1, dry_run=False, filter_department=None):
    ...
    with app.app_context():
        ...
        query = db.session.query(Participant).outerjoin(EmailLog, 
        ...
        logger.info(f"📈 Success rate: {(success_count / len(participants)) * 100:.1f}%")
    ...
    with app.app_context():
        ...
        for participant in participants:
            try:
                ...
                msg['From'] = smtp_username
                msg['To'] = participant.email
                msg['Subject'] = f'BUNDOKAI 2024 - Lembrete: Evento em {days_before} dia(s)!'
                ...
                logger.error(f"❌ Failed to send reminder to {participant.nome}: {str(e)}")
                error_count += 1
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Send QR Code emails to BUNDOKAI participants')
    parser.add_argument('--type', choices=['qr', 'reminder'], default='qr',
