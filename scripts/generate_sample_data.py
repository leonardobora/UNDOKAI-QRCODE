...
FIRST_NAMES = [
    'Ana', 'Antonio', 'Beatriz', 'Carlos', 'Daniela', 'Eduardo', 'Fernanda', 'Gabriel',
    'Helena', 'Igor', 'Julia', 'Leandro', 'Mariana', 'Nicolas', 'Olivia', 'Pedro',
    'Rafaela', 'Rodrigo', 'Sofia', 'Thiago', 'Valentina', 'William', 'Yasmin', 'Zara'
]
...
DEPENDENT_NAMES = [
    ...
    'Lucas', 'Luiza', 'Miguel', 'Alice', 'Arthur', 'Laura', 'Gabriel', 'Manuela',
    'Rafael', 'Isabela', 'Guilherme', 'Helena', 'Mateus', 'Valentina', 'Nicolas', 'Sophia'
]
    ...
    try:
        with app.app_context():
            for data in participant_data_list:
                ...
                existing = Participant.query.filter_by(email=data['participant']['email']).first()
                if existing:
                    logger.info(f"Participant {data['participant']['nome']} already exists, skipping...")
                    continue
            ...
            db.session.commit()
            logger.info(f"Successfully created {len(created_participants)} participants")
    ...
    try:
        with app.app_context():
            ...
            for participant in selected_participants:
                ...
                checkin_time = datetime.now() - timedelta(hours=hours_ago, minutes=minutes_ago)
        ...
        with app.app_context():
            ...
            for participant in selected_participants:
                ...
                for item in selected_items:
                    ...
                    delivery_time = datetime.now() - timedelta(days=days_ago, hours=hours_ago)
        ...
        with app.app_context():
            ...
            for participant in participants:
                ...
                for _ in range(email_count):
                    ...
                    hours_ago = random.randint(0, 23)
                    sent_time = datetime.now() - timedelta(days=days_ago, hours=hours_ago)
                    ...
                    email_log = EmailLog(
                        ...
                        sent_at=sent_time,
                        status=status,
                        opened_at=sent_time + timedelta(hours=random.randint(1, 48)) if status == 'opened' else None
                    )
