...
class Participant(db.Model):
    ...
    # Relationships
    dependents = db.relationship('Dependent', backref='participant', lazy=True, cascade='all, delete-orphan')
...
class DeliveryItem(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    nome = db.Column(db.String(100), nullable=False)
    categoria = db.Column(db.String(50), nullable=False)  # Festa, Cesta Básica, Brinquedos, Material Escolar
    ...
    def items_delivered(self):
        return sum(log.quantidade for log in self.delivery_logs if log.status == 'delivered')
...
class EmailLog(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    participant_id = db.Column(db.Integer, db.ForeignKey('participant.id'), nullable=False)
    email_type = db.Column(db.String(50), nullable=False)  # qr_delivery, reminder, confirmation
