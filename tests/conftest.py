...
def sample_dependent():
    ...
    dependent = Dependent(nome="Maria Silva", idade=8)
    return dependent
...
def db_with_data(test_app):
    ...
    with test_app.app_context():
        ...
        dependent1 = Dependent(
            nome="Maria Silva", idade=8, participant_id=participant.id
        )
        dependent2 = Dependent(
            nome="Pedro Silva", idade=12, participant_id=participant.id
        )
        ...
        # Add a check-in
        checkin = CheckIn(
            participant_id=participant.id, station="main", operator="admin"
        )
