...
from app import db
from models import CheckIn, DeliveryItem, DeliveryLog, Dependent, Participant
    ...
    def test_register_participant_duplicate_email(self, client, test_app):
        ...
        with test_app.app_context():
            ...
            participant1 = Participant(
                nome="First User", email="duplicate@lightera.com", qr_code="QR001"
            )
            ...
            assert response.status_code in [
                200,
                302,
            ]  # Either show form with error or redirect
...
class TestAPIRoutes:
    ...
    def test_lookup_participant_by_email_success(
        self, client, test_app, sample_participant
    ):
        ...
        with test_app.app_context():
            ...
            response = client.post(
                "/api/lookup_participant_by_email",
    ...
    def test_validate_qr_success(self, client, test_app, sample_participant):
        ...
        with test_app.app_context():
            ...
            response = client.post("/api/validate_qr", json={"qr_code": "QR123456"})
            assert response.status_code == 200
    ...
    def test_validate_qr_invalid_code(self, client, test_app):
        ...
        with test_app.app_context():
            response = client.post("/api/validate_qr", json={"qr_code": "INVALID123"})
            assert response.status_code == 200
    ...
    def test_manual_checkin_success(self, client, test_app, sample_participant):
        ...
        with test_app.app_context():
            ...
            response = client.post(
                "/api/manual_checkin", json={"participant_id": sample_participant.id}
            )
            assert response.status_code == 200
    ...
    def test_manual_checkin_invalid_participant(self, client, test_app):
        ...
        with test_app.app_context():
            response = client.post(
                "/api/manual_checkin", json={"participant_id": 99999}
            )
            assert response.status_code == 200
...
class TestErrorHandling:
    ...
    def test_api_invalid_json(self, client, test_app):
        ...
        with test_app.app_context():
            response = client.post(
                "/api/validate_qr", data="invalid json", content_type="application/json"
            )
...
class TestRouteIntegration:
    ...
    def test_complete_participant_flow(self, client, test_app):
        ...
        with test_app.app_context():
            ...
            response = client.post("/api/validate_qr", json={"qr_code": qr_code})
            data = response.get_json()
            assert data["success"] is True
